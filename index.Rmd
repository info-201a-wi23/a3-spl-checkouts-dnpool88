...
The first value I found interesting is month with the highest # of checkouts:
```{r, echo = FALSE}
df %>% group_by(CheckoutMonth) %>% filter(max(Checkouts))
```
And the second value I found interesting is month with the lowest # of checkouts:
```{r, echo = FALSE}
df %>% group_by(CheckoutMonth) %>% filter(min(Checkouts))
```
...
```{r}
df %>% group_by(MaterialType) %>% filter(Title == "Educated : a memoir / Tara Westover.", Total = sum(Checkouts))
```
